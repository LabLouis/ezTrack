...
intensity_threshold=0.5; % threshold for binary image conversion; set to 0.5 in absence of lid; set to 0.4 in presence of the lid
...
%cd('/Users/mlouis/Dropbox (CRG)/SHARED/CSHL15, practicals/150630/larva 3, no light/')

%% READING IMAGES: Load the image files
filesArray=dir('BASLER*.bmp');
...
for frameNumber=1:1:length(filesArray)
    ...
    if DISH_CENTER_DIST_MM(frameNumber)>(radius_mm-edge_tolerance)
        display('Wall contacted before end of trajecotry')
        break
    end
